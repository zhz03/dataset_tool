...
def _load_T(path, mm_to_m=True):
        with open(path) as f:
            j = json.load(f)
        R = np.asarray(j["rotation"])                # 3×3
        t = np.asarray(j["translation"])
        # if mm_to_m:                                  # set False if JSON already uses metres
        #     t = t / 1000.0
        T = np.eye(4)
        T[:3, :3] = R
        T[:3, 3]  = t
...
class ProjLidar2Lidar:
    ...
    def single_radar2lidar(self, pcd_path1, radar_path, yaml_file,
        ...
        for i, radar_key in enumerate(config_key2):
            try:
                ...
                tf_matrix = self.construct_tf_matrix(lidar_pose , radar_pose)
    ...
    def single_comb(self,
                pcd_path1, pcd_path2,
                calib_json1, calib_json2,
                save_path=None, vis_flag=False):
        # 1. point-clouds
        pcd1 = self.load_point_cloud(pcd_path1, mode="xyzi")
        ...
        # 2. calibration → transforms
        T1_S2W = _load_T(calib_json1)     # LiDAR-1 → World
        T2_S2W = _load_T(calib_json2)     # LiDAR-2 → World

        # 3. relative transform LiDAR-2 → LiDAR-1
        T2_to_1 = np.linalg.inv(T1_S2W) @ T2_S2W

        # 4. align second cloud
        pcd2.transform(T2_to_1)

        # 5. merge / save / visualise
        combined = pcd1 + pcd2
        if save_path:
            self.save_point_cloud(combined, save_path)
        if vis_flag:
            self.visualize_point_cloud(combined)

    
    def merge_many(self, pcd_paths, calib_paths,      # lists of equal length
               target_idx=0,                # 0 → use first sensor as reference
               save_path=None, vis=False):
        assert len(pcd_paths) == len(calib_paths) >= 2
        n = len(pcd_paths)
        # --- load all transforms ---
        Ts_S2W = [_load_T(p) for p in calib_paths]
        T_target_inv = np.linalg.inv(Ts_S2W[target_idx])
        merged = o3d.geometry.PointCloud()
        for i in range(n):
            pcd = o3d.io.read_point_cloud(pcd_paths[i])
            color = np.random.rand(3)          # random colour per sensor
            pcd.paint_uniform_color(color)

            if i != target_idx:
                T_rel = T_target_inv @ Ts_S2W[i]   # sensor-i → target
                pcd.transform(T_rel)

            merged += pcd                       # concatenate
        if save_path:
            self.save_point_cloud(merged, save_path)
        if vis:
            self.visualize_point_cloud(merged)
...
if __name__ == "__main__":
    ...
    # proj_radar2lidar.single_comb('/media/guest/pred_1/RCooper_data/data/106-105/105/seq-0/lidar/1692685344.949990.pcd', 
    # '/media/guest/pred_1/RCooper_data/data/106-105/106/seq-0/lidar/1692685344.949958.pcd', '/media/guest/pred_1/RCooper_data/calib/lidar2world/105.json', '/media/guest/pred_1/RCooper_data/calib/lidar2world/106.json', save_path=None, vis_flag=True)

    pcds = [
            # # First intersection
            #  '/media/guest/pred_1/RCooper_data/data/106-105/105/seq-0/lidar/1692685344.949990.pcd',
            #  '/media/guest/pred_1/RCooper_data/data/106-105/106/seq-0/lidar/1692685344.949958.pcd',
            #  # Second intersection 
            #  '/media/guest/pred_1/RCooper_data/data/116-115/115/seq-0/lidar/1690532674.349753.pcd',
            #  '/media/guest/pred_1/RCooper_data/data/116-115/116/seq-0/lidar/1690532674.349866.pcd',
            #  '/media/guest/pred_1/RCooper_data/data/117-118-120-119/117/seq-0/lidar/1693204527.273104.pcd',
            #  '/media/guest/pred_1/RCooper_data/data/117-118-120-119/118/seq-0/lidar/1693204527.354066.pcd',
            #  '/media/guest/pred_1/RCooper_data/data/117-118-120-119/119/seq-0/lidar/1693204527.354835.pcd',
            #  '/media/guest/pred_1/RCooper_data/data/117-118-120-119/120/seq-0/lidar/1693204527.249897.pcd',
             # Third intersection
             '/media/guest/pred_1/RCooper_data/data/136-137-138-139/136/seq-0/lidar/1693908913.283240.pcd',
             '/media/guest/pred_1/RCooper_data/data/136-137-138-139/137/seq-0/lidar/1693908913.339286.pcd',
             '/media/guest/pred_1/RCooper_data/data/136-137-138-139/138/seq-0/lidar/1693908913.340532.pcd',
             '/media/guest/pred_1/RCooper_data/data/136-137-138-139/139/seq-0/lidar/1693908913.285267.pcd'
             ]
    calibs = [
            # # First intersection
            #  '/media/guest/pred_1/RCooper_data/calib/lidar2world/105.json', 
            #  '/media/guest/pred_1/RCooper_data/calib/lidar2world/106.json',
            # # Second intersection
            #  '/media/guest/pred_1/RCooper_data/calib/lidar2world/115.json',
            #  '/media/guest/pred_1/RCooper_data/calib/lidar2world/116.json',
            #  '/media/guest/pred_1/RCooper_data/calib/lidar2world/117.json',
            #  '/media/guest/pred_1/RCooper_data/calib/lidar2world/118.json',
            #  '/media/guest/pred_1/RCooper_data/calib/lidar2world/119.json',
            #  '/media/guest/pred_1/RCooper_data/calib/lidar2world/120.json',
            # Third intersection
             '/media/guest/pred_1/RCooper_data/calib/lidar2world/136.json',
             '/media/guest/pred_1/RCooper_data/calib/lidar2world/137.json',
             '/media/guest/pred_1/RCooper_data/calib/lidar2world/138.json',
             '/media/guest/pred_1/RCooper_data/calib/lidar2world/139.json'
             ]
    proj_radar2lidar.merge_many(pcds, calibs,target_idx=0, save_path='/media/guest/pred_1/RCooper_data/Intersections/136-139_intersection.pcd', vis=True)